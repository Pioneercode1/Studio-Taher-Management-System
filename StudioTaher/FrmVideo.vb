...
Public Class FrmVideo
    ...
    Public Sub myconnaction()
        ...
        Catch ex As SqlException
            MsgBox(ex.Message, "ستوديو وفيديو طاهر")
        Finally
            conn.Close()
        End Try
    End Sub
    ...
    Private Sub FrmVideo_Activated(sender As Object, e As EventArgs) Handles Me.Activated
        Try
            With dgrview
                ...
                .Columns(4).HeaderText = "طلبات العميل"
                .Columns(5).HeaderText = "المبلغ الاجمالى"
                .Columns(6).HeaderText = "المبلغ المدفوع"
                .Columns(7).HeaderText = "المبلغ المتبقى"
                .Columns(8).HeaderText = "تاريخ التصوير"
                .Columns(9).HeaderText = "تاريخ التسليم"
                .Columns(10).HeaderText = "الموظف"
                .Columns(11).HeaderText = "المصورين"
                .Columns(12).HeaderText = "حالة التسليم"
                .Columns(13).HeaderText = "ملاحظات"
                .Columns(14).HeaderText = "هدية المحل"
            End With
        Catch ex As Exception
            MessageBox.Show(ex.Message, "ستوديو وفيديو طاهر")
        End Try
    ...
    Private Sub btnsave_Click(sender As Object, e As EventArgs) Handles btnsave.Click
        Try
            cmd = New SqlCommand("INSERT INTO CustomerVideo(CusNmae,CusAdrress,CusNotes,AllPrise,CusPhone,CusOrder,CusGavet,JetPrise,SetPrise,FriDate,SecDate,AboutOrder,OrderBy,CameraMan)" & _
           "VALUES('" & txtCusname.Text & "','" & txtCusAdrress.Text & "','" & txtCusNotes.Text & "','" & txtAllPrise.Text & "','" & txtCusPhone.Text & "','" & empname.Text & "','" & txtgavet.Text & "','" & txtJetPrise.Text & "','" & txtSetPrise.Text & "','" & DtFriDate.Value.ToString("yyyy/MM/dd") & "','" & DtSecDate.Value.ToString("yyyy/MM/dd") & "','" & cbAboutOrder.Text & "','" & txtOrderBy.Text & "','" & txtCameraMan.Text & "')", conn)
    ...
    Private Sub btnupdate_Click(sender As Object, e As EventArgs) Handles btnupdate.Click
        Try
            cmd = New SqlCommand("UPDATE CustomerVideo SET CusNmae = @CusNmae,CusAdrress = @CusAdrress,CusNotes = @CusNotes,AllPrise = @AllPrise,CusPhone = @CusPhone,CusOrder = @CusOrder,Cusgavet = @Cusgavet, JetPrise = @JetPrise,SetPrise = @SetPrise,FriDate = @FriDate,SecDate = @SecDate,AboutOrder = @AboutOrder,OrderBy = @OrderBy,CameraMan = @CameraMan WHERE CusId = " & txtCusId.Text, conn)
            With cmd.Parameters
                .AddWithValue("@CusNmae", txtCusname.Text).DbType = DbType.String
                .AddWithValue("@CusAdrress", txtCusAdrress.Text).DbType = DbType.String
                ...
                .AddWithValue("@Cusgavet", txtgavet.Text).DbType = DbType.String
                ...
                .AddWithValue("@CameraMan", txtCameraMan.Text).DbType = DbType.String
            End With
            If conn.State = ConnectionState.Open Then conn.Close()
            conn.Open()
            cmd.ExecuteNonQuery()
    ...
    Private Sub btndelete_Click(sender As Object, e As EventArgs) Handles btndelete.Click
        Try
            If MessageBox.Show("هل انت تريد حذف هذا العميل", "ستوديو وفيديو طاهر", MessageBoxButtons.YesNo) = Windows.Forms.DialogResult.Yes Then
                cmd = New SqlCommand("DELETE FROM CustomerVideo WHERE CusId = '" & txtCusId.Text & "'", conn)
                If conn.State = ConnectionState.Open Then conn.Close()
                conn.Open()
                cmd.ExecuteNonQuery()
                conn.Close()
    ...
    Private Sub txtcustsearch_TextChanged(sender As Object, e As EventArgs) Handles txtcustsearch.TextChanged
        Try
            ...
            conn = New SqlConnection("Data Source=.\SQLExpress; INITIAL CATALOG=StudioTaher;INTEGRATED SECURITY=True;")
            ds = New DataSet
            da = New SqlDataAdapter(String.Format("select * from CustomerVideo where CusNmae like '%{0}%'", txtcustsearch.Text), conn)
            da.Fill(ds, "CustomerVideo")
            dv = New DataView(ds.Tables("CustomerVideo"))
            cur = CType(Me.BindingContext(dv), CurrencyManager)
            dgrview.DataSource = dv
